...
class AppStore extends Component {
  state = {
    isActiveTabId: tabsList[0].tabId,
    searchInput: '',
  }
  ...
  getFilterAppLists = () => {
    ...
    const filteredAppList = appsList.filter(
      eachAppItem => eachAppItem.category === isActiveTabId,
    )
  ...
  render() {
    ...
    const filteredAppList = this.getFilterAppLists()
    const searchResults = filteredAppList.filter(eachSearchApp =>
      eachSearchApp.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
